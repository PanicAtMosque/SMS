<?php
  require 'config.php';
  $conn = connection();
  $customer_info = $_REQUEST['id'];
  $cat = "SELECT pro_grpid as g from product where pro_firmid = $customer_info GROUP BY pro_grpid ORDER BY g ASC";
  $cat = $conn->query($cat);
  // $sql = "SELECT * FROM product p inner join category c on c.cat_id=p.pro_grpid inner join type t on t.ty_id=p.pro_typeid inner join customer_info i on i.customer_id=p.pro_firmid ORDER BY cat_name ASC ";
  ...
  ?>
    ...
    <?php 
      ...
      foreach ($ty as $value) { $t = $value['ty_id']; ?>
        ...
        <?php $price = "SELECT * FROM product WHERE pro_typeid = $t AND pro_grpid = $id AND pro_firmid = $customer_info";
